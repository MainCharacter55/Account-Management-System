...
def main():
    ...
    if manager.accounts:
        ...
        print(
            f"✅ {len(manager.accounts)}件のアカウントをロードしました。基準通貨: {manager.base_currency}"
        )
    else:
        # DBにデータが見つからなかった: 初期設定のプロンプトに進む。
        print("💡 初期セットアップを開始します (データが見つかりませんでした)。")
        # 基準通貨の初期設定。
        manager.base_currency = input(
            "基準通貨を設定してください (例: JPY/USD/...):").upper()
        ...
        try:
            ...
            if manager.add_account(account_name, initial_balance,
                                   account_currency):
        ...
        if choice == "1":
            ...
            while True:
                ...
                change_currency = input(
                    "基準通貨を変更するには新しい通貨を入力してください (または 'Enter' で続行):").upper()
                # 新しい通貨が入力されたかチェック。
                if change_currency == "":
                    break
            ...
            for name, account_obj in manager.accounts.items():
                ...
                try:
                    if source_curr != base:
                        ...
                        converted_balance = converter.get_converted_amount(
                            balance, source_curr, base)
                        if converted_balance is None:
                            print(
                                f"⚠️ {name} ({source_curr}) の換算に失敗しました。スキップします。"
                            )
                            continue  # APIが失敗した場合はこのアカウントをスキップ。
                        # 小数点以下2桁に丸めて表示。
                        print(
                            f"{name:<10}: {balance:<10} {source_curr} -> {converted_balance.quantize(Decimal('.01'))} {base}"
                        )
                    else:
...
# ----------------------------------------------------------------------------------------------------
            ...
            try:
                ...
                # 現在のアカウントを除外。
                switch_options = [
                    name for name in available_accounts
                ...
                for i, name in enumerate(switch_options):
                    ...
                    print(
                        f"{i+1}. {name} ({account_obj.currency}) - 残高: {account_obj.get_balance()} {account_obj.currency}"
                    )
                ...
                if source_currency != destination_currency:
                    ...
                    converted_amount = converter.get_converted_amount(
                        transfer_amount, source_currency, destination_currency)
                ...
                # 送金額は常に送金元の通貨建て。
                if not source_account.set_withdraw(
                        f"'{destination_name}' ({destination_currency}) への送金",
                        transfer_amount):
                ...
                destination_account.set_deposit(
                    f"'{current_account}' ({source_currency}) からの送金",
                    converted_amount)
...
# ----------------------------------------------------------------------------------------------------
            ...
            try:
                ...
                deposit = DepositTransaction(current_account, amount,
                                             account_obj.currency, subject)
...
# ----------------------------------------------------------------------------------------------------
            ...
            if not history:
                ...
                for i, record in enumerate(history):
                    ...
                    print(
                        f"{transaction_type:<10} | 件名:{subject:<15} | 金額:{amount} {curr}"
                    )
...
# ----------------------------------------------------------------------------------------------------
            ...
            # 現在のアカウントを除外。
            switch_options = [
                name for name in available_accounts if name != current_account
            ]
            if not switch_options:
                # 現在のアカウントのみ、またはアカウントがない場合。
...
# ----------------------------------------------------------------------------------------------------
            ...
            # 1. 確認を求める。
            conformation = input(
                f"現在のアカウント'{current_account}'を削除しますか? ('C'で確定):").upper()
            if conformation != "C":
